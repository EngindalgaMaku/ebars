...
class UserSession:
    ...
    def get_recent_sessions(self, limit: int = 100) -> List[Dict[str, Any]]:
        """
        Get recent sessions (both active and expired)
    ...
    def get_session_count(self, user_id: int = None, active_only: bool = False) -> int:
        """
        ...
        """
        try:
            query = "SELECT COUNT(*) as count FROM user_sessions s"
            params = []
            conditions = []
            ...
            if active_only:
                conditions.append("s.expires_at > CURRENT_TIMESTAMP")
