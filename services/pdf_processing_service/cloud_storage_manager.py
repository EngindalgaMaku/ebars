...
# Standalone configuration for PDF service - no dependency on main project config
class SimpleConfig:
    ...
    def database_config(self):
        """Simple database config for PDF service"""
        return {
            'storage_bucket': os.getenv('GCS_BUCKET_NAME', 'nimble-gearing-476415-t2-rag3-data'),
...
class CloudStorageManager:
    ...
    def upload_blob(self, blob_name: str, data: bytes) -> bool:
        """
        Upload binary data to cloud storage.
        
        Args:
            blob_name: Name/path of the blob in cloud storage
        ...
        if not self.is_cloud or not self._bucket:
            ...
            try:
                local_temp_dir = Path("/tmp/rag3/cache")
                local_temp_dir.mkdir(parents=True, exist_ok=True)
                
                with open(local_temp_dir / blob_name.replace('/', '_'), 'wb') as f:
                    f.write(data)
                return True
            except Exception as e:
                self.logger.error(f"Failed to save blob locally: {e}")
                return False
        
        try:
            blob = self._bucket.blob(blob_name)
            blob.upload_from_string(data)
            self.logger.info(f"Blob uploaded to GCS: {blob_name}")
            return True
        except Exception as e:
            self.logger.error(f"Failed to upload blob to GCS: {e}")
            return False

    def download_blob(self, blob_name: str) -> Optional[bytes]:
        """
        Download binary data from cloud storage.
        ...
        """
        if not self.is_cloud or not self._bucket:
            # For local development, read from temp directory
            try:
                local_temp_dir = Path("/tmp/rag3/cache")
                local_file = local_temp_dir / blob_name.replace('/', '_')
                ...
                self.logger.error(f"Failed to read blob locally: {e}")
                return None
        ...
        try:
            ...
            self.logger.error(f"Failed to download blob from GCS: {e}")
            return None
