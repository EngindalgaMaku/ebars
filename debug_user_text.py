...
def test_user_text():
    """Test LLM text cleaning with user's exact text."""
...
Bu anlayışla hareket ederek doğayı sevmeli ve korumalıdır."""
    ...
    print("🧹 Testing LLM Preprocessor with User's Exact Text...")
    print(f"📝 Original text length: {len(user_text)} characters")
    print()
    print("📝 Original text (first 500 chars):")
    print("=" * 80)
    print(user_text[:500] + "..." if len(user_text) > 500 else user_text)
    print("=" * 80)
    print()
    ...
    try:
        ...
        print("✅ LLM Preprocessor Results:")
        print(f"📝 Cleaned text length: {len(cleaned_text)} characters")
        print(f"📝 Length ratio: {len(cleaned_text)/len(user_text):.2f}")
        print()
        ...
        print(f"  Original: {len(user_text)} characters")
        print(f"  Cleaned:  {len(cleaned_text)} characters")
        print(f"  Ratio:    {len(cleaned_text)/len(user_text):.2f}")
        
        if len(cleaned_text) > len(user_text) * 1.2:
            print("  ⚠️ WARNING: Text expanded significantly!")
        elif len(cleaned_text) < len(user_text) * 0.8:
            print("  ⚠️ WARNING: Text shrunk significantly!")
        else:
            print("  ✅ Length change is reasonable")
